...
class cogtest:
    ...
    @commands.command(pass_context = True, no_pm = True)
    async def editrole(self, ctx, role, color:discord.Colour = None, separated = None, taggable = None):
        s = ctx.message.server
        bc = discord.utils.get(ctx.message.server.roles, name = 'Bot-Commander')
        u = ctx.message.author 
        if bc in u.roles:
            await self.bot.edit_role(s, name = role:discord.Role, colour = color, hoist = separated, mentionable = taggable)
